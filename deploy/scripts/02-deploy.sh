...
[[ -x "$GO" ]] || chet "Chưa có Go ở $GO. Chạy 01-setup-server.sh trước."
...
# này chưa tồn tại; công cụ tự tạo nếu tài khoản MySQL đủ quyền (01-setup-server.sh
...
install -m 700 "$APP_DIR/deploy/scripts/03-backup.sh"  /usr/local/sbin/selliotech-sao-luu
install -m 700 "$APP_DIR/deploy/scripts/04-restore.sh" /usr/local/sbin/selliotech-phuc-hoi
...
install -m 700 "$APP_DIR/deploy/scripts/create-admin.sh" /usr/local/sbin/selliotech-tao-admin
...
install -m 755 "$APP_DIR/deploy/motd/99-selliotech-backup" /etc/update-motd.d/99-selliotech-sao-luu
