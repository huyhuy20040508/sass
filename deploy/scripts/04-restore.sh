...
if [[ -z "$BAN" ]]; then
    ...
    bash /usr/local/sbin/selliotech-sao-luu liet-ke 2>/dev/null \
        || bash "$APP_DIR/deploy/scripts/03-backup.sh" liet-ke
    cat <<'HD'
...
if [[ -n "$MA_LUU" && -n "$MA_NAY" && "$MA_LUU" != "$MA_NAY" ]]; then
    ...
    vang "  Lược đồ có thể đã đổi. Sau khi phục hồi, chạy lại 02-deploy.sh để"
...
if (( LAM_DB )); then
    ...
    if [[ -f "$NGUON/nen-tang.sql.gz" ]]; then
        ...
        if (( MY_ROOT )); then
            ...
            # sẵn (01-setup-server.sh tạo) — xoá từng bảng như bên trên.
...
if (( LAM_ANH )); then
    if [[ -f "$NGUON/anh.tar" ]]; then
        ...
        mv "$MOI" "$ANH_DIR"
        # Quyền y như bước 6 của 02-deploy.sh: chủ sở hữu là app (PHP ghi),
...
if (( CA_ENV )); then
    if [[ -f "$NGUON/cauhinh.tar.gz" ]]; then
        ...
        vang "  Laravel còn cache config cũ — chạy 02-deploy.sh để nạp lại."
    else
...
Việc nên làm ngay:
    ...
    2. Nếu mã nguồn lệch với bản sao lưu:  sudo bash $APP_DIR/deploy/scripts/02-deploy.sh
