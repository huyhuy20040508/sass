{{-- Màn Tồn kho chi nhánh hiện tại dựng theo khuôn v2 (warehouse: index + list).
     ...
     Dữ liệu do BranchStockController đẩy sang: $rows, $groups, $filters,
     $meta, $chiNhanh, $categories. --}}
@extends('v2::layouts.master')

@section('title', \App\Http\Controllers\BranchStockController::TITLE_PAGE)

@php
    $C = \App\Http\Controllers\BranchStockController::class;
    ...
    $tenKho = \App\Services\CurrentBranch::ten();
...
@section('content')
                            ...
                            <tbody>
                                ...
                                @forelse ($rows as $r)
                                    ...
                                    <tr class="item {{ $keDong }}" data-cn="{{ $shopId }}"
                                        ...
                                        @if ($lo)
                                            @include('v2::branch-stock._lot_cell', ['l' => $lo[0]])
                                        @else
                                            <td colspan="3"></td>
                                        @endif

                                        <td class="text-center">
                                            {{-- shop_id gắn theo TỪNG DÒNG: bảng bày nhiều kho cùng lúc nên
                                                 "chi nhánh đang làm việc" không nói đúng dòng nào. --}}
                                            <button type="button" class="btn btn-sm btn-so-kho" title="Sổ kho"
                                                data-variant="{{ $maDong }}" data-shop="{{ $shopId }}"
                                                data-ten="{{ $r['product_name'] ?? '' }}">
                                    ...
                                    @foreach (array_slice($lo, 1) as $l)
                                        <tr class="item batch-{{ $maDong }} {{ $keDong }}" style="display: none"
                                            ...
                                            @include('v2::branch-stock._lot_cell', ['l' => $l])
    ...
    <div class="offcanvas offcanvas-end offcanvas-custom" tabindex="-1" id="offcanvasDetail"
        ...
        <div class="modal" id="modalSoKho" style="padding-inline: 0 !important">
        <div class="modal-dialog modal-dialog-centered modal-lg mx-auto">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">Sổ kho — <span id="so-kho-ten"></span></h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="table-responsive">
                        <table class="table-list-warehouse" style="width: 100%">
                            <thead>
                                <tr>
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.table-list-warehouse.none_desktop .item[data-id]', function () {
            ...
            // ---------- Sổ kho ----------
        const URL_TON_KHO = @json(url('/admin/inventory'));
        const TX_TYPES = @json(\App\Http\Controllers\BranchStockController::TX_TYPES);

        // Ghi chú và mã chứng từ do người dùng gõ, nên phải thoát trước khi nối chuỗi HTML.
        const thoat = (s) => $('<div>').text(s == null ? '' : s).html();
        ...
        $(document).on('click', '.btn-so-kho', function () {
            const $nut = $(this);
            const $body = $('#so-kho-body');

            $('#so-kho-ten').text($nut.data('ten') || '');
            $body.html('<tr><td colspan="6" class="text-center py-3 text-muted">Đang tải…</td></tr>');
            $('#modalSoKho').modal('show');

            $.ajax({
                url: URL_TON_KHO + '/' + $nut.data('variant') + '/history',
                data: { shop_id: $nut.data('shop') },
                headers: { Accept: 'application/json' },
            }).done(function (r) {
                const ds = (r && r.data) || [];
                if (!ds.length) {
                    $body.html('<tr><td colspan="6" class="text-center py-3 text-muted">Kho này chưa phát sinh lượt nào.</td></tr>');

                    return;
                }

                $body.html(ds.map(function (d) {
                    const sl = Number(d.quantity || 0);

                    return '<tr>'
                        + '<td class="text-left">' + ngayGioVN(d.created_at) + '</td>'
                        + '<td class="text-left">' + thoat(TX_TYPES[d.type] || d.type) + '</td>'
                        + '<td class="text-left">' + thoat(d.reference_code || d.note) + '</td>'
                        + '<td class="text-right">' + (sl > 0 ? '+' : '') + soVN(sl) + '</td>'
                        + '<td class="text-right">' + soVN(d.quantity_before) + '</td>'
                        + '<td class="text-right">' + soVN(d.quantity_after) + '</td>'
                        + '</tr>';
                }).join(''));
            }).fail(function (x) {
                const cau = (x.responseJSON || {}).message || 'Không tải được sổ kho.';
                toastr.error(cau);
                $body.html('<tr><td colspan="6" class="text-center py-3 text-danger">' + thoat(cau) + '</td></tr>');
            });
        });
