{{-- Màn Quản lý chi nhánh dựng theo khuôn v2 (system/branch/index + list).
     ...
     Khác bản v2 ở ba chỗ, đều vì backend bên này khác:
       ...
       - v2 có ô "Ngày hết hạn" (sys_branches.expired_time do nhà cung cấp quản
         ...
         SubscriptionExpiry — nên bỏ ô đó thay vì bày một ô luôn trống.
       - ô QR đặt hàng online đổi thành "Link truy cập": API trả `access_link`,
         không trả sẵn ảnh QR như v2.

     Dữ liệu do BranchController đẩy sang: $list, $tatCa, $meta, $filters,
     $dangLamId, $tong, $dangMo, $stt. --}}
@extends('v2::layouts.master')

@section('title', \App\Http\Controllers\BranchController::TITLE_PAGE)
...
@php
    $C = \App\Http\Controllers\BranchController::class;
...
@push('scripts')
    <script>
        ...
        const LOAI = @json(\App\Http\Controllers\BranchController::LOAI);
        const NHA_CC = @json(\App\Http\Controllers\BranchController::NHA_CUNG_CAP_ETAX);
        const CHU_TOI_DA = {{ $MAX }};
        const TRANG_THAI_CHU = @json(\App\Http\Controllers\BranchController::TRANG_THAI);
