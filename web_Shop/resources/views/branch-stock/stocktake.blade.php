...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* `fixed` + phần trăm: để `auto` thì tên hàng dài hút hết chỗ, sáu nhãn cột
           bị bóp xuống hai dòng. Cắt "…" không dùng được ở đây — tờ giấy in ra
           phải đọc được tên cột thì người đi đếm mới biết đang điền vào ô nào. */
        ...
        /* Cộng đủ 100 khi hiện đủ cột. Ẩn cột Giá vốn / Tồn sổ sách (đếm mù) thì
           trình duyệt tự chia lại phần còn thiếu theo tỉ lệ — không phải sửa gì. */
        .st-c-stt   { width: 4%; }
        .st-c-sku   { width: 12%; }
        .st-c-name  { width: 14%; }
        .st-c-var   { width: 12%; }
        .st-c-sys   { width: 11.5%; }
        .st-c-cost  { width: 9%; }
        .st-c-count { width: 14.5%; }
        .st-c-diff  { width: 11.5%; }
        .st-c-note  { width: 11.5%; }
        /* Dòng cao để còn chỗ viết tay — đây là mục đích của cả tờ giấy này. */
        tbody td { height: 9mm; }
        .c { text-align: center; }
        .r { text-align: right; }
        /* Mã dài thì xuống dòng chứ không cắt: đây là thứ người đếm dò theo. */
        .st-sku { font-family: 'Courier New', monospace; font-size: 11px; overflow-wrap: anywhere; }
        ...
        /* Bật cột Giá vốn là 9 cột chen trong 190mm — chỗ cho nhãn không còn đủ.
           Cho nhãn xuống dòng ở RIÊNG trạng thái đó: hai dòng vẫn đọc được, cắt
           mất chữ thì không. Số tiền cũng phải gãy được, không thì nó tràn ô. */
        .show-cost th { white-space: normal; }
        .st-c-cost { overflow-wrap: anywhere; }
        ...
        @foreach($theoKho as $khoID => $kho)
            ...
            <div class="st-kho {{ $loop->first ? '' : 'st-kho-moi' }}">
                ...
                <table>
                    <thead>
                        <tr>
                            <th class="st-c-stt">STT</th>
                            <th class="st-c-sku">SKU</th>
                            <th class="st-c-name">Sản phẩm</th>
                            <th class="st-c-var">Phân loại</th>
                            <th class="col-sys st-c-sys">Tồn sổ sách</th>
                            <th class="col-cost st-c-cost">Giá vốn</th>
                            <th class="st-c-count">Số đếm thực tế</th>
                            <th class="st-c-diff">Chênh lệch</th>
                            <th class="st-c-note">Ghi chú</th>
                        </tr>
                    </thead>
                    <tbody>
