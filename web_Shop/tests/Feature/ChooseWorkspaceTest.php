 ...
 */
class ChooseWorkspaceTest extends TestCase
{
    /**
     * Xoá bộ nhớ TRONG-MỘT-REQUEST của CurrentBranch trước mỗi bài.
     *
    ...
    {
        $nho = new \ReflectionProperty(CurrentBranch::class, 'nho');
    ...
    /**
     * Một cửa nhưng NHIỀU chi nhánh: vẫn phải thấy màn này.
     *
     * Vỏ Thu ngân không cho đổi chi nhánh, nên đây là chỗ duy nhất thu ngân chọn
     * được kho hôm nay. Bỏ màn đi là khoá họ vào đúng một kho.
     */
    public function test_mot_cua_nhieu_chi_nhanh_van_thay_man_chon(): void
    {
        $this->fakeApi([
            ['id' => 1, 'name' => 'Kho trung tâm'],
            ['id' => 2, 'name' => 'Kho quận 7'],
        ]);
    ...
    /**
     * Một cửa, một chi nhánh: KHÔNG hỏi gì nữa, vào thẳng quầy.
     *
     * Màn chọn lúc này chỉ còn đúng một ô để bấm — một nút thừa chen giữa lượt
     * đăng nhập và ca làm. Đường đăng xuất vẫn còn trong menu tài khoản của vỏ
     * Thu ngân nên không mất lối ra khi gõ nhầm tài khoản.
     */
    public function test_mot_cua_mot_chi_nhanh_vao_thang(): void
    {
        $this->fakeApi([['id' => 1, 'name' => 'Kho trung tâm']]);
    ...
    /** Tiệm chưa khai chi nhánh nào cũng vậy: một cửa thì đi thẳng. */
    public function test_mot_cua_khong_chi_nhanh_nao_vao_thang(): void
    {
        $this->fakeApi();
    ...
    public function test_nguoi_khong_co_cua_nao_di_thang(): void
    {
        ...
        // mà phiên vẫn qua được chốt đăng nhập (xem Workspace::tuPhien).
     ...
     *
     * Bỏ khoá là "chưa chọn", và CurrentBranch ghim chi nhánh đầu tiên đè lên
