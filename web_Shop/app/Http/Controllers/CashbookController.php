...
class CashbookController extends Controller
{
     ...
     * vai — Quản lý (`quan_ly`) và Thu ngân (`thu_ngan`), xem StaffController::NHAN_CUA.
    ...
    public function dinhKem(Request $request)
    {
        ...
        return response()->json(['url' => ImageStore::put($request->file('file'), 'cashbook')]);
    }
    ...
    protected function send(callable $call, string $success, Request $request)
    {
        ...
        return $res->successful()
            ? $this->traLoiHopThoai($request, true, $success, fn () => redirect()->route('admin.cashbook.index'))
            : $this->traLoiHopThoai(
                $request,
                false,
