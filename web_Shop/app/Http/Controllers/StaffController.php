...
class StaffController extends Controller
{
    ...
    {
        $ve = redirect()->route('admin.staff.index');
     ...
     *
     * Lý do cũ ("dựng .xlsx phải kéo thêm một thư viện") không còn: App\Support\XlsxOrder
     ...
     * mất sạch đầu số điện thoại, mà người nhận không có cách nào biết. XlsxOrder ghi
    ...
    public function uploadAnh(Request $request)
    {
        ...
        return response()->json(['url' => ImageStore::put($request->file('anh'), 'staff')]);
    }
     ...
     *
     * Bảng chỉ bày người của chi nhánh đang đứng (xem BranchPerTab). Sửa một
    ...
    protected function send(callable $call, string $success, ?callable $sauKhiLuu = null)
    {
        ...
        if ($res->successful()) {
            ...
            // lần đăng nhập sau. Đây đúng là chỗ "đáng hỏi" mà Workspace nói tới.
            Workspace::lamMoi();
            ...
            return redirect()->route('admin.staff.index')->with('success', $cau);
        }
