...
// QuyenMacDinhTheoCua trả bộ quyền TỐI THIỂU cho những cửa vừa giao.
//
// Vì sao cần: cửa vào (`users.access_areas`) và quyền theo chức năng
// (`user_permissions`) là hai sổ tách rời — cửa nói "mở được khu nào", quyền nói
// "được làm việc gì". Hộp Nhân sự chỉ tích được CỬA, còn màn phân quyền thì chưa
// có bản v2, nên tài khoản lập qua hộp ấy ra đời với đúng 0 quyền: thu ngân mới
// đăng nhập được vào quầy nhưng quét mã vạch nhận 403 (don-hang.xem) và bấm
// Thanh toán cũng 403 (don-hang.them). Không có đường nào trên giao diện để chữa.
//
// Đây chỉ là ĐIỂM XUẤT PHÁT, đúng tinh thần NhomDungSan: cửa hàng sửa lại được
// bất cứ lúc nào qua màn phân quyền, và lượt gieo chỉ chạm vào tài khoản CHƯA
// được giao việc gì.
//
// Cửa Quản lý cố ý KHÔNG có bộ mặc định ở đây: "toàn quyền" là quyết định phải
// do người ta bấm, không phải thứ tự rơi vào tài khoản lúc lập hồ sơ.
func QuyenMacDinhTheoCua(cua []string) []string {
	for _, c := range cua {
		if strings.TrimSpace(c) == CuaThuNgan {
			return QuyenThuNgan()
		}
	}
