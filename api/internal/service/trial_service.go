...
//
// Đây là bản dịch sang HTTP của `cmd/subscriptions ky | gia-han | huy`. Công cụ dòng
...
// maCuaHangRe khớp `cmd/create-admin` và ô đầu tiên của màn hình đăng nhập 3 ô.
...
func (s *dungThuService) taoKhachHangMoi(
	...
	//    nhập được, và ký hợp đồng bù bằng `cmd/subscriptions ky` là xong.
...
func (s *dungThuService) KyHopDong(ctx context.Context, quyen domain.QuyenApp, req dto.KyHopDongRequest) (dto.HopDongItem, error) {
	...
	// vào sổ đó (cửa hàng dựng bằng `cmd/create-admin` chẳng hạn).
	if err := s.hopDong.UpsertKhachHang(ctx, domain.PlatformTenant{
		ID:     ch.ID,
		Code:   ch.Ma,
		Name:   ch.Ten,
...
//
// Bản dịch sang HTTP của `cmd/subscriptions thu-tien`, giữ nguyên mọi luật của nó —
...
type dieuKhoanChot struct {
	...
	// lại biên bản chép giống bảng đối chiếu của `cmd/subscriptions ky`. Ghi một hợp
...
//
// Đường này KHÔNG cho khai tay, khác `cmd/subscriptions ky`. Chủ ý: đây là màn hình
...
func chuanHoaCuaHang(req dto.KhachHangMoiChung) (domain.CuaHangMoi, map[string]string) {
	...
	//
	// Đuôi .local theo đúng quy ước của `cmd/create-admin`, và cố ý là một tên miền
