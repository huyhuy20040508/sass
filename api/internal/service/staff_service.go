...
func (s *nhanSuService) datCuaVao(ctx context.Context, userID uint, req *dto.NhanSuRequest, actor Actor) error {
	...
	if err := s.users.SetCuaVao(ctx, userID, req.Quyen, actor); err != nil {
		return err
	}

	return s.gieoQuyenMacDinh(ctx, userID, req.Quyen)
}

// gieoQuyenMacDinh cấp bộ quyền tối thiểu của cửa cho tài khoản CHƯA được giao
// việc gì.
//
// Cửa vào và quyền theo chức năng là hai sổ tách rời: tích "Thu ngân" mới chỉ
// mở được khu quầy, còn mỗi đường trong đó lại đòi một quyền riêng. Hộp Nhân sự
// không tích được quyền, mà màn phân quyền thì chưa có bản v2 — nên trước lượt
// sửa này, thu ngân lập qua hộp ấy đăng nhập vào quầy rồi đứng im: quét mã vạch
// 403 (don-hang.xem), bấm Thanh toán 403 (don-hang.them), và không có đường nào
// trên giao diện để cấp cho họ.
//
// CHỈ gieo khi tài khoản đang trắng: có một quyền đã tick, hoặc mang cờ toàn
// quyền, nghĩa là chủ tiệm đã tự quyết — ghi đè lên đó là lấy mất quyết định của
// họ, và tệ hơn là làm lượt Sửa hồ sơ âm thầm đặt lại quyền về mặc định.
func (s *nhanSuService) gieoQuyenMacDinh(ctx context.Context, userID uint, cua []string) error {
	macDinh := domain.QuyenMacDinhTheoCua(cua)
	if len(macDinh) == 0 {
		return nil
	}

	toanQuyen, dangCo, err := s.quyen.QuyenRiengCuaNguoi(ctx, userID)
	if err != nil {
		return err
	}
	if toanQuyen || len(dangCo) > 0 {
		return nil
	}

	return s.quyen.DatQuyenChoNguoi(ctx, userID, macDinh)
}
