...
//
//	go run ./cmd/subscriptions danh-sach [--app order]
//	go run ./cmd/subscriptions ky      --ma-cua-hang quochuy --goi chuoi --gia 9990000 --chi-nhanh 5 --tai-khoan 0 --san-pham 0
//	go run ./cmd/subscriptions gia-han --ma-cua-hang quochuy --thang 12
//	go run ./cmd/subscriptions huy     --ma-cua-hang quochuy --ly-do "khách dừng dịch vụ"
//	go run ./cmd/subscriptions thu-tien --ma-cua-hang quochuy --ma-giao-dich FT2608130001
//
...
func dungCachDung() {
  ...
  Hợp đồng của khách trên sổ nền tảng.
    ...
    go run ./cmd/subscriptions danh-sach [--app <mã>] [--trang-thai trial|active|past_due|canceled]
    go run ./cmd/subscriptions ky      --ma-cua-hang <mã> --goi <mã gói> [--app <mã>] [--chu-ky thang|nam]
                                  [--gia <số>] [--chi-nhanh <số>] [--tai-khoan <số>] [--san-pham <số>]
                                  [--ten-mien-rieng co|khong] [--dung-thu <ngày>] [--thang <số>] [--ghi-chu "..."]
    go run ./cmd/subscriptions gia-han --ma-cua-hang <mã> [--app <mã>] --thang <số>
    go run ./cmd/subscriptions huy     --ma-cua-hang <mã> [--app <mã>] --ly-do "<lý do>"
    go run ./cmd/subscriptions thu-tien --ma-cua-hang <mã> [--app <mã>] [--so-tien <số>]
		...
		if strings.Contains(err.Error(), "uq_subscriptions_current") {
			return fmt.Errorf(
				"cửa hàng %q đã có hợp đồng còn hiệu lực cho phần mềm %q.\n"+
					"        Gia hạn:  go run ./cmd/subscriptions gia-han --ma-cua-hang %s --app %s --thang 12\n"+
					"        Đổi gói:  huỷ hợp đồng cũ trước, rồi ký lại:\n"+
					"                  go run ./cmd/subscriptions huy --ma-cua-hang %s --app %s --ly-do \"đổi gói\"",
	...
	case errors.Is(err, sql.ErrNoRows):
		return nil, fmt.Errorf(
			"bảng giá của phần mềm %q không có gói %q chu kỳ %q.\n"+
				"        Xem có gì đang bán:  go run ./cmd/subscriptions danh-sach --app %s\n"+
...
func huy(nenTang *sql.DB, ts thamSo) error {
	...
	fmt.Printf("    Gỡ tên miền:  go run ./cmd/domains danh-sach   rồi   ten-mien xoa --host <tên miền>\n\n")
	...
	case errors.Is(err, sql.ErrNoRows):
		return 0, hd, fmt.Errorf(
			"cửa hàng %q chưa có hợp đồng còn hiệu lực cho phần mềm %q.\n"+
				"        Ký mới:  go run ./cmd/subscriptions ky --ma-cua-hang %s --app %s --goi <mã gói>",
...
func danhSach(nenTang *sql.DB, ts thamSo) error {
	...
	if so == 0 {
		...
		fmt.Println("      go run ./cmd/subscriptions ky --ma-cua-hang <mã> --goi <mã gói>")
	}
	fmt.Println()
...
func thuTien(nenTang *sql.DB, ts thamSo) error {
		...
		if nguonKy == "nối tiếp kỳ đã trả" {
			return fmt.Errorf(
				"hợp đồng này đã được trả tiền tới %s, đúng bằng hạn hiện tại — không còn kỳ nào để thu.\n"+
					...
					"            go run ./cmd/subscriptions gia-han --ma-cua-hang %s --app %s --thang <số>",
		...
		if strings.Contains(err.Error(), "uq_invoices_ky") {
			return fmt.Errorf(
				"đã có lần thu nào đó ghi cho kỳ bắt đầu %s của hợp đồng này.\n"+
					"        Xem lại sổ:  go run ./cmd/subscriptions danh-sach --app %s\n"+
	...
	fmt.Printf("      go run ./cmd/subscriptions gia-han --ma-cua-hang %s --app %s --thang <số>\n\n",
		ts.MaCuaHang, ts.App)
