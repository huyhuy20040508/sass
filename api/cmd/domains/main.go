...
//
//	go run ./cmd/domains danh-sach
//	go run ./cmd/domains them --ma-cua-hang quochuy --host quochuy.selliotech.store --chinh
//	go run ./cmd/domains xoa  --host quochuy.selliotech.store
//
...
func dungCachDung() {
  ...
  Quản lý sổ tên miền của nền tảng.
    ...
    go run ./cmd/domains danh-sach
    go run ./cmd/domains tu-dong --ma-cua-hang <mã> [--xem-truoc]
    go run ./cmd/domains them --ma-cua-hang <mã> --host <tên miền> [--chinh]
    go run ./cmd/domains them --ma-cua-hang <mã> --host <tên miền> --dac-cach "<lý do>"
    go run ./cmd/domains xoa --host <tên miền>
	...
	case errors.Is(err, sql.ErrNoRows):
		...
		return fmt.Errorf(
			"cửa hàng %q chưa có thuê bao %s nào trong sổ nền tảng, nên chưa chứng minh được đã mua gói gì.\n"+
				...
				"            go run ./cmd/domains them --ma-cua-hang %s --host <tên miền> --dac-cach \"<lý do>\"",
