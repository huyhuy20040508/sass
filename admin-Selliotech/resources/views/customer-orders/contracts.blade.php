...
@section('content')
    ...
    <div class="page-bar">
        ...
        @if (count($hopDong))
            <form method="POST" action="{{ route('platform.customer-orders.contracts.xuat') }}"
                    ...
                    @foreach ($hopDong as $hd)
                        ...
                        <tr data-id="{{ data_get($hd, 'id') }}" data-han="{{ $con_lai }}"
                            ...
                            <td class="act">
                                ...
                                <a class="btn-mini"
                                   href="{{ route('platform.customer-orders.contracts.chi-tiet', data_get($hd, 'id')) }}">
                                    Chi tiết
                                </a>
        ...
        @else
            <div class="empty">
                ...
                @elseif ($la_dung_thu)
                    ...
                    @else
                        Hợp đồng dùng thử cũng tạo được bằng công cụ <span class="mono">cmd/subscriptions</span> trên máy chủ.
                    @endif
                @else
    ...
    @if ($ghiDuoc)
        @include('customer-orders.partials.contract-actions', ['laDungThu' => $la_dung_thu])
    @endif

    @if (! $la_dung_thu && $ghiDuoc)
        @include('customer-orders.partials.collect-payment')

        @if (count($plans))
            @include('customer-orders.partials.add-official')
        @endif
    @endif

    @if ($la_dung_thu && $ghiDuoc && count($plans))
        @include('customer-orders.partials.add-trial', [
            'plans' => $plans,
            'moLai' => $mo_lai_form,
            'oForm' => $o_form_them,
        ])
